...
public class AtraccionDAOImpl implements AtraccionDAO{
	...
	public int insert(Atraccion atraccion) {//throws SQLException {
		...
		try{
		String sql = "INSERT INTO atracciones (tipoAtr, nombre, precio, cupo, tiempoRequerido)"
				+ " VALUES (?, ?, ?, ?, ?)";
		Connection conn = ConnectionProvider.getConnection();

		PreparedStatement statement = conn.prepareStatement(sql);
		statement.setString (1, atraccion.getTipoDeAtraccion().toString());
	...
	public Atraccion findByAtraccionname(String username) { // throws SQLException {
	...
	public ArrayList<Atraccion> findByPreferenciasUsuario(Usuario usuario) {
		...
		try {
			String sql = "SELECT * FROM atracciones WHERE "
					+ "(tipoAtr = ? AND cupo > 0 AND precio >= ? AND tiempoRequerido >= ?) "
